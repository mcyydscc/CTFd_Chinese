...
def new():
    ...
    if num_teams_limit and num_teams >= num_teams_limit:
        abort(
            403,
            description=f"已达到团队最大人数 ({num_teams_limit}) 人. 请加入其他团队.",
        )

    user = get_current_user_attrs()
    if user.team_id:
        errors.append("你已经在一个团队里了。你不能加入另一个团队.")
    ...
    if request.method == "GET":
        ...
        if existing_team:
            errors.append("那个队名已经有人用了")
        if not teamname:
            errors.append("那个队名无效")
        ...
        if valid_website is False:
            errors.append("网站必须是以http或https开头的正确URL")
        if valid_affiliation is False:
            errors.append("请提供一个更短的组织关系。")
