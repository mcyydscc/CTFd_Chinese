...
def setup():
    ...
    if not config.is_setup():
        ...
        if request.method == "POST":
            ...
            if emails:
                errors.append("该电子邮件已被使用")
            if pass_short:
                errors.append("选一个长一点的密码")
            if pass_long:
                errors.append("选一个短一点的密码")
            if name_len:
                errors.append("选择更长的用户名")
    ...
    <div class="col-md-6 offset-md-3">
        ...
        <h3 class="text-center">
            <p>一个很酷的平台来自 <a href="https://ctfd.io">ctfd.io</a></p>
            <p>在社交媒体上关注我们：</p>
        ...
        <h4 class="text-center">
            <a href="admin">点击这里</a> 登录并管理你的CTF平台吧
        </h4>
    </div>
</div>"""
            page.content = index
...
def settings():
    ...
    if is_teams_mode() and get_current_team() is None:
        ...
        infos.append(
            markup(
                f'为了参与比赛，你必须 <a href="{team_url}">加入或创建一个团队</a>.'
            )
        )
    ...
    if get_config("verify_emails") and not user.verified:
        ...
        infos.append(
            markup(
                ...
                f'重新发送确认电子邮件请 <a href="{confirm_url}">点击这里</a>.'
            )
        )
